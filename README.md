...
| 02 | Servicios con Canvas sincronizado al scroll y narrativa progresiva | En revisión |
...
- `services.ts`: secuencia Canvas de Servicios, carga progresiva, narrativa por etapas y fallback estático.
- `specialized-accordion.ts`: acordeón de selección única para servicios especializados; la primera respuesta aparece abierta y las transiciones respetan el movimiento reducido.
...
Las transiciones entre secciones utilizan el atributo reutilizable `data-section-panel`: la sección anterior reduce escala, opacidad y nitidez mientras entra la siguiente. Todas las divisiones son rectas y continúan el flujo normal del documento, sin radios, solapamientos ni contenedores auxiliares. `data-section-transition-surface` limita el efecto al contenido visual de Servicios y mantiene intacto el flujo que ScrollTrigger utiliza para calcular el scroll. En móvil, el Hero dirige el escalado y desenfoque de salida exclusivamente a la capa fotográfica mediante `data-section-transition-background`, conservando nítidos el texto, los controles y las estadísticas. Servicios usa `position: sticky` para su escena y un ScrollTrigger exclusivo para sincronizar el Canvas, sin pinSpacing adicional. Cobertura mantiene timelines y ScrollTriggers independientes, con iluminación reactiva, siete puntos provinciales y un panel accesible para explorar sus 84 cantones. Las animaciones se desactivan cuando el usuario prefiere movimiento reducido, sin perder las interacciones.

### Secuencia de Servicios

La entrada «Todo acceso tiene una solución.» y el bloque de servicios especializados permanecen en HTML. Entre ambos hay una escena Canvas de cinco etapas (residencial, automotriz, cajas fuertes, portones y comercial). Los títulos, descripciones, características y CTA son HTML accesible superpuesto, nunca texto dibujado en Canvas. El bloque final presenta nueve preguntas y respuestas específicas sobre trabajos de cerrajería mediante acordeones nativos `<details>`/`<summary>`; no sustituye la futura sección de preguntas frecuentes generales. Su composición editorial usa un título de gran formato que acompaña el scroll en escritorio, una llave decorativa tenue y un enlace de contacto; la fotografía lateral se retiró. La primera respuesta se muestra abierta y, al abrir otra, la anterior se cierra. Cada fila tiene estados de hover y activo; el control circular «+» gira a «×» mientras la respuesta se abre o cierra con una transición de altura, opacidad y desplazamiento. La interacción respeta `prefers-reduced-motion` y actualiza las medidas de ScrollTrigger después de cada cambio de altura.

Mientras no exista la secuencia definitiva, el componente utiliza cinco fotografías WebP horizontales de Servicios, reconstruidas mediante outpainting a partir de las tomas originales, que se conservan para el encuadre móvil. Las versiones horizontales miden 1672 × 941 px (16:9 aproximado) y se optimizan a 1600 px para escritorio; los originales móviles se exportan a WebP de hasta 800 px. La herramienta de generación integrada no entregó 4K nativo, por lo que no se amplían artificialmente solo para declarar 3840 × 2160 px. Los prompts y las fuentes están documentados en [`OUTPAINTING.md`](src/assets/images/services/OUTPAINTING.md). El renderizador interpola 16 estados de cámara por servicio (80 estados visuales en total), con transición entre fotografías. Este recurso provisional mantiene el peso bajo y permite validar la experiencia sin producir decenas de imágenes artificiales. ScrollTrigger controla el índice en ambos sentidos con `scrub`; el Canvas solo se repinta cuando cambia el estado o el tamaño. La escena dura 300 `svh` de recorrido en escritorio y 260 `svh` en móvil; el sticky se libera antes de la lista especializada sin crear un pin spacer.

La primera imagen se solicita de inmediato; las siguientes se preparan progresivamente cuando la escena se aproxima. La decodificación admite como máximo dos imágenes simultáneas, el DPR del Canvas está limitado a 1.5 y su superficie a 2.8 millones de píxeles. Con movimiento reducido, `Save-Data`, conexión 2G, memoria de 2 GB o menos o Canvas no disponible, se muestran tarjetas estáticas con las mismas acciones. También son el fallback sin JavaScript.

Para instalar la secuencia definitiva, coloca archivos WebP consecutivos `frame-0001.webp`, `frame-0002.webp`, etc. en `public/images/services/sequence/desktop/` y sus equivalentes optimizados en `public/images/services/sequence/mobile/`. Luego ajusta únicamente `frameCount` en `src/data/service-sequence.ts`. El renderizador cambia automáticamente de los cinco fotogramas clave a los archivos individuales; precarga los fotogramas próximos al scroll y limita a 14 los decodificados en memoria. Ambos directorios deben tener el mismo número de frames. Los recortes deben mantener la continuidad de cámara entre imágenes para que la animación final se perciba fluida.
